...
class CourseController extends Controller
{
  ...
  if (auth()->check()) {
    $key = $req->query('query');
        ...
        $data['instructors'] = $search->clone()->select('created_by')->with(['instructor' => function ($query) {
   $loggedInUser = auth()->user();
        $loggedInUserLatitude = $loggedInUser->latitude;
        $loggedInUserLongitude = $loggedInUser->longitude;

            $query->select('id', 'name', 'latitude', 'longitude', 
                DB::raw("SQRT(POW(69.1 * (latitude - $loggedInUserLatitude), 2) + POW(69.1 * ($loggedInUserLongitude - longitude) * COS(latitude / 57.3), 2)) AS distance"))
                ->having('distance', '<', 100); // Adjust the distance threshold as needed
