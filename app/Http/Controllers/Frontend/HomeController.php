...
class HomeController extends Controller
{
    ...
    public function Rawdi()
    {

        try {

            if (Cache::has('Rawdi')) {
                $featuredCourses = Cache::get('Rawdi');

//  dd( $featuredCourses);
// Cache::forget('Rawdi');

            } else {
                $featuredCourses = $this->course->model()->active()->where('course_category_id',"1")->visible()->orderBy('id', 'DESC')->limit(8)->get();

                Cache::put('Rawdi', $featuredCourses);
            }
            $data['courses'] = $featuredCourses;
            $data['title'] = ___('frontend.Rawdi');
            $data['url'] = route('courses') . '?type=featured';
            $view = view('frontend.ajax.home.Rawdi', compact('data'))->render();
            $response['content'] = $view;
...
// dd(1);
        try {
            if (Cache::has('Basic')) {
                $featuredCourses = Cache::get('Basic');
            } else {
                $featuredCourses = $this->course->model()->active()->where('course_category_id',"2")->visible()->orderBy('id', 'DESC')->limit(8)->get();
                Cache::put('Basic', $featuredCourses);
            }
            $data['courses'] = $featuredCourses;
            $data['title'] = ___('frontend.Basic');
            $data['url'] = route('courses') . '?type=featured';
            $view = view('frontend.ajax.home.Basic', compact('data'))->render();
            $response['content'] = $view;
            $response['message'] = ___('frontend.Featured Courses');
            return $this->responseWithSuccess(___('alert.Data found.'), $response); // return success response from ApiReturnFormatTrait
        } catch (\Throwable $th) {
dd($th);
    ...
    public function Preparatory()
    {
           ...
           try {
            if (Cache::has('preparatory')) {
                $featuredCourses = Cache::get('preparatory');
            } else {
                $featuredCourses = $this->course->model()->active()->where('course_category_id',"3")->visible()->orderBy('id', 'DESC')->limit(8)->get();
                Cache::put('Preparatory', $featuredCourses);
            }
            $data['courses'] = $featuredCourses;
            $data['title'] = ___('frontend.Preparatory');
            $data['url'] = route('courses') . '?type=featured';
            $view = view('frontend.ajax.home.preparatory', compact('data'))->render();
            $response['content'] = $view;
            $response['message'] = ___('frontend.Preparatory Courses');
            return $this->responseWithSuccess(___('alert.Data found.'), $response); // return success response from ApiReturnFormatTrait
        } catch (\Throwable $th) {
dd($th);
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'));
    ...
    {
        try {
            if (Cache::has('secondary')) {
                $featuredCourses = Cache::get('secondary');
// Cache::forget('secondary');
            } else {
                $featuredCourses = $this->course->model()->active()->where('course_category_id',"4")->visible()->orderBy('id', 'DESC')->limit(8)->get();
                Cache::put('secondary', $featuredCourses);
            }
            $data['courses'] = $featuredCourses;
            $data['title'] = ___('frontend.secondary');
            $data['url'] = route('courses') . '?type=featured';
            $view = view('frontend.ajax.home.secondary', compact('data'))->render();
            $response['content'] = $view;
            $response['message'] = ___('frontend.secondary  Courses');
            return $this->responseWithSuccess(___('alert.Data found.'), $response); // return success response from ApiReturnFormatTrait
        } catch (\Throwable $th) {
// dd($th);
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'));
    ...
    public function Bilingualschools()
    {
        try {
            if (Cache::has('Bilingualschools')) {
                $featuredCourses = Cache::get('Bilingualschools');
// Cache::forget('Bilingualschools');
            } else {
                // $featuredCourses = $this->featuredCourse->model()->active()->with('course')->where('course_category_id',"1")->orderBy('id', 'ASC')->limit(8)->get();
                $featuredCourses = $this->course->model()->active()->visible()->where('course_category_id',"5")->orderBy('id', 'DESC')->limit(8)->get();
            }
            $data['courses'] = $featuredCourses;
            $data['title'] = ___('frontend.Bilingual schools');
            $data['url'] = route('courses') . '?type=featured';
            $view = view('frontend.ajax.home.Basic', compact('data'))->render();
            $response['content'] = $view;
            $response['message'] = ___('frontend.Bilingual schools');
            return $this->responseWithSuccess(___('alert.Data found.'), $response); // return success response from ApiReturnFormatTrait
        } catch (\Throwable $th) {
dd( $th);
            return $this->responseWithError(___('alert.something_went_wrong_please_try_again'));
